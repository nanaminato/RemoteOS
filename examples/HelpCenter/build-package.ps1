...
$project = Join-Path $projectRoot 'RemoteOS.Example.HelpCenter.csproj'
dotnet build $project --configuration $Configuration
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$output = Join-Path $projectRoot "bin\$Configuration\net10.0"
$staging = Join-Path $output '.roapp-staging'
$package = Join-Path $output 'RemoteOS.Example.HelpCenter.roapp'
...
Copy-Item -LiteralPath (Join-Path $output 'RemoteOS.Example.HelpCenter.dll') -Destination (Join-Path $staging 'lib\net10.0\RemoteOS.Example.HelpCenter.dll')
