...
- `NetworkInspectorWindowService` 是 RemoteOS Shell 的系统服务，直接通过 `IWindowManager` 创建 `remoteos.system.network-inspector` 窗口；它不是内置应用，也不是可安装的 `.roapp`。
...
`Client.Services.Diagnostics` 中的内部契约由系统窗口直接使用：
