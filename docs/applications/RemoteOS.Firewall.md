...
3. 用户可启用/禁用 UFW、修改默认策略、新增、查看、修改或删除编号规则。启用和禁用按钮会随当前状态互斥；规则表显示编号、动作、方向、协议、来源、目标和端口。UFW 为任意地址规则自动生成的相邻 IPv4/IPv6 项会合并显示为一条逻辑规则，更新或删除时会一起处理。页面始终提示这些变更可能中断当前会话。
