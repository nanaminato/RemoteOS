...
2. 创建、编辑、申请、导入等需要输入或修改数据的操作，若字段不止少量且内容可能增长，必须在模态对话框中完成，而不是占用资源列表或概览页面。主页面只提供入口、状态与结果。
3. 模态对话框的字段、说明、校验结果和长文本输入必须放在独立的可滚动内容区；取消、预检、返回、保存、确认、提交等决策按钮固定在对话框右下角的页脚，位于滚动区之外并始终可见。对危险操作仍须遵循第 5 节的预检、影响说明与明确确认要求。
4. 长任务的取消必须请求服务端取消相应 `OperationId`，客户端停止等待只是辅助行为；关闭对话框不得让服务端任务丢失状态。预检、进度、错误和成功结果使用稳定问题码并由客户端本地化。
