...
1. 默认最小权限。新增宿主机能力前，在 `AppPermissions` 定义稳定权限，按“读取 / 管理 / 安装或提权影响”拆分，并接入应用权限界面和服务端授权。
...
5. Endpoint 验证认证、授权、输入范围、所有权/资源可见性、路径规范化与速率/大小限制；客户端校验只用于体验，不能替代服务器校验。
...
- 每项存储数据指定真源、用户/Workspace 隔离、迁移和保留期。运行时事实数据（如进程、容器）不复制为永久真源。
