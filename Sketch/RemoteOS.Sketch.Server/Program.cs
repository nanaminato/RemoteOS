...
docker.MapPost("/containers", (DockerContainerCreateRequest request, SketchMockStore store) => Results.Ok(store.CreateContainer(request)));
docker.MapPut("/containers/{id}", (string id, DockerContainerUpdateRequest request, SketchMockStore store) => Results.Ok(store.UpdateContainer(id, request)));
...
docker.MapPost("/images/pull", (DockerImagePullRequest request, SketchMockStore store) => Results.Ok(store.PullImage(request)));
...
docker.MapPost("/networks", (DockerNetworkCreateRequest request, SketchMockStore store) => Results.Ok(store.CreateNetwork(request)));
docker.MapDelete("/networks/{id}", (string id, bool? confirmed, SketchMockStore store) => Results.Ok(store.DeleteNetwork(id, confirmed ?? false)));
docker.MapPost("/volumes", (DockerVolumeCreateRequest request, SketchMockStore store) => Results.Ok(store.CreateVolume(request)));
docker.MapDelete("/volumes/{name}", (string name, bool? confirmed, SketchMockStore store) => Results.Ok(store.DeleteVolume(name, confirmed ?? false)));
